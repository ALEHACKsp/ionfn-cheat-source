...
VOID EndScene(ImGuiWindow& window) {
	...
	if (ShowMenu) {
		...
		ImGui::GetStyle().FrameRounding = 4.0f;
			...
			ImGui::Begin("IONFN V3", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize); {
			...
			if (ImGui::Button("Visuals", ImVec2(180.0f, 0.0f)))
				switchTabs = 1;
			ImGui::SameLine(0.0, 2.0f);
			if (ImGui::Button("Credits", ImVec2(180.0f, 0.0f)))
				switchTabs = 2;
			...
			case 2:
				...
				ImGui::Text("--");
				ImGui::Text(" ");
				//ImGui::Checkbox(("(soon) No Spread"), &config_system.item.NoSpreadAimbot);
				//ImGui::Checkbox(("(soon) Sniper TP"), &config_system.item.BulletTP);
				//ImGui::Checkbox(("soon) Spinbot (CAPSLOCK)"), &config_system.item.SpinBot);
				//ImGui::Checkbox(("(soon) Rapid Fire (use Burst!)"), &config_system.item.RapidFire);
				//ImGui::Checkbox(("Crosshair"), &config_system.item.Crosshair);
			...
			case 2:
				ImGui::Text(" ");
				ImGui::Text("---Credits---");
				ImGui::Text("OWNER HackerGod#8696 and drknowi on hotspot#0001");
				//ImGui::Checkbox(("(soon) No Spread"),);
				//ImGui::Checkbox(("(soon) Sniper TP"),);
				if (ImGui::Button("Join ION FN Discord"))
				{
					system("start https://discord.gg/dkB6XBV");
				}
				
				if (ImGui::Button("ION FN YouTube"))
				
				{
					system("start https://www.youtube.com/channel/UCOxmcWa1TGa3DQ1Rww8lY5Q?view_as=subscriber");
				}
				
				if (ImGui::Button("ION FN Website"))
				{
					system("start http://ion-fn.bplaced.net/news.php");
				}
...
__declspec(dllexport) HRESULT PresentHook(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags) {
	...
	do {
		...
		for (auto li = 0UL; li < ReadDWORD(world, Offsets::Engine::World::Levels + sizeof(PVOID)); ++li) {
			...
			for (auto ai = 0UL; ai < ReadDWORD(level, Offsets::Engine::Level::AActors + sizeof(PVOID)); ++ai) {
				...
				if (wcsstr(name.c_str(), L"PlayerPawn_Athena_C") || wcsstr(name.c_str(), L"PlayerPawn_Athena_Phoebe_C") || wcsstr(name.c_str(), L"BP_MangPlayerPawn") || wcsstr(name.c_str(), L"HoagieVehicle_C")) {
					...
					AddMarker(window, width, height, localPlayerLocation, pawn, "Llama", ImGui::GetColorU32({ 255.0,255.0,255.0,1 }));
				}
				if (config_system.item.Chest && wcsstr(name.c_str(), L"Tiered_Chest") && !((ReadBYTE(pawn, Offsets::FortniteGame::BuildingContainer::bAlreadySearched) >> 7) & 1)) {
					AddMarker(window, width, height, localPlayerLocation, pawn, "Chest", ImGui::GetColorU32({ 255.0,255.0,255.0,1 }));
				}
					...
					AddMarker(window, width, height, localPlayerLocation, pawn, "Ammo Box", ImGui::GetColorU32({ 255.0,255.0,255.0,1 }));
				}
				else if (config_system.item.chopper && wcsstr(name.c_str(), L"HoagieVehicle_C")) {

					AddMarker(window, width, height, localPlayerLocation, pawn, "Chopper", ImGui::GetColorU32({ 255.0,255.0,255.0,1 }));
				}
				else if (config_system.item.boat && wcsstr(name.c_str(), L"MeatballVehicle_L")) {

					AddMarker(window, width, height, localPlayerLocation, pawn, "Boat", ImGui::GetColorU32({ 255.0,255.0,255.0,1 }));
				}
			}
		}
		...
		for (auto pawn : playerPawns)
		{
			...
			auto color = ImGui::GetColorU32({ 255.0,255.0,255.0,1 });
			FVector viewPoint = { 0 };
			...
			if (ReadDWORD(state, 0xE60) == localPlayerTeamIndex) {
				...
				color = ImGui::GetColorU32({ 0.65,0,0,1 });
		...
		if (config_system.item.DrawAimbotFOV) {
			window.DrawList->AddCircle(ImVec2(width / 2, height / 2), config_system.item.AimbotFOV, ImGui::GetColorU32({ 255.0,255.0,255.0,1 }), 128);
		}
